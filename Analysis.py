...
def app():
    ...
    def dataframe():
        ...
        df1 = pd.read_csv('Cleaned_datas.csv')
        return df,df1
    df,df1 = dataframe()
    ...
    if choice == 'Explore of Weekly Sales':
           ...
           st.bar_chart(sales, x='Store', y=['Weekly_Sales', 'Markdown'])
           ...
           sales = df.groupby(['Dept'])[['Weekly_Sales']].sum().reset_index()
           ...
           data = df[df['year'] == radio]
           dataframe = data.groupby('day')[('Weekly_Sales')].sum().reset_index()
           st.bar_chart(dataframe, x = 'day',y = 'Weekly_Sales')
        ...
        sales = df.groupby(['Store'])[['Markdown','Weekly_Sales']].sum().reset_index()
